...
use crate::{NatStream, Message, http::handle_http, utils, SSHStatus};
            ...
            select! {
                ...
                _ = stream.readable() => match Message::from_stream(&stream).await {
					Ok(msg) => match msg {
						...
						Some(msg) if msg.is_http() => match handle_http(&msg).await {
							Ok(res) => {
								// Send to server
								self.sender.write().await.send(Message::new_http(msg.tracing_id, res.as_bytes().to_vec())).unwrap();
							},
							Err(e) => {
								error!("Redirect http request failed: {:?}", e);
								if e.source().is_some() {
									if "operation timed out" == format!("{}", e.source().unwrap()) {
										self.sender.write().await.send(Message::http_504(msg.tracing_id)).unwrap();
									} else {
										self.sender.write().await.send(Message::http_502(msg.tracing_id)).unwrap();
									}
								} else {
									self.sender.write().await.send(Message::http_502(msg.tracing_id)).unwrap();
								}
				...
				msg = ssh_rx.recv() => match msg {
					Some(msg) => {
						...
						// Send to server
						self.sender.write().await.send(msg).unwrap();
					},
					None => {}
				},
